...
export class ReclaimAgent {
    ...
    async updateUserPreferences(userId, preferences) {
        ...
        try {
            ...
            await this.redis.set(`user:${userId}:preferences`, JSON.stringify(updated));
        }
        catch (err) {
    ...
    async analyzeProduct(url, userId) {
        ...
        if (productName && productName.length > 3 && productName.toLowerCase() !== "amazon.com") {
            try {
                ...
                const usedResult = await this.searchAlternatives(productName, `used pre-owned secondhand cheaper price buy sell`);
                if (usedResult.cached) {
                    cacheStats.alternativesCached = true;
                    cacheStats.totalCacheHits++;
                }
                // Search for refurbished version (cheaper)
                const refurbishedResult = await this.searchAlternatives(productName, `refurbished renewed open box cheaper price discount`);
                ...
                const similarCheaperResult = await this.searchAlternatives(productName, `cheaper alternative similar product same function lower price budget`);
                if (similarCheaperResult.cached && !cacheStats.alternativesCached) {
                    cacheStats.alternativesCached = true;
                    cacheStats.totalCacheHits++;
                }
                // Search for generic/off-brand alternatives (usually cheaper)
                const genericResult = await this.searchAlternatives(productName, `generic alternative off brand cheaper price same use`);
                ...
                for (const result of allResults) {
                    ...
                    const hasProductMatch = productKeywords.some(keyword => title.includes(keyword) || content.includes(keyword) || url.includes(keyword));
                    if (!hasProductMatch && productKeywords.length > 0)
                        continue;
                    ...
                    if (alternatives.length >= 8)
                        break; // Limit to 8 best alternatives
                }
            }
            catch (err) {
        ...
        if (manipulationClaims.length > 0 && this.tavilyClient) {
            try {
                const verifiedClaims = await Promise.all(manipulationClaims.map(async (claim) => {
        ...
        const recommendation = await this.generateRecommendation(productInfo, manipulationSignals, manipulationClaims, alternatives, userPrefs, productName);
    ...
    detectManipulation(productInfo) {
        const signals = [];
        const claims = [];
    ...
    async verifyClaim(claim, productName) {
        if (!this.tavilyClient) {
    ...
    async generateRecommendation(productInfo, manipulationSignals, manipulationClaims, alternatives, userPrefs, productName) {
...
${score >= 70 ? 'This appears reasonable, but always consider: Do you really need this? Could you borrow, rent, or find it used?' : ''}
    ...
    async setUserPreferences(userId, prefs) {
        ...
        try {
            await this.redis.set(`user:${userId}:preferences`, JSON.stringify(prefs));
        }
        catch (err) {
...
//# sourceMappingURL=reclaim-agent.js.map